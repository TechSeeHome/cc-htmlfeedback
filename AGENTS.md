...
marketplace, plus the DesignHub publishing subsystem. Things a useful review of this
repo must know:
...
- `designhub/gas/widget.js` is likewise generated by `node designhub/build-designhub.js`.
...
- DesignHub publish scripts and e2e helpers are **ESM** (`.mjs`).
- `designhub/gas/` files run in Google Apps Script, not Node - do not suggest Node-only
  APIs there.
...
- Coverage gate: c8, 75% lines minimum, run via `npm run test:cov` (both the root and
  designhub suites). New logic in `lib/`, `server.js`, or `designhub/gas/lib/` should
  come with tests.

## Review priorities

1. Hand-edits to generated files (`plugins/cc-htmlfeedback/{server.js,lib,feedback-widget.js}`,
   `designhub/gas/widget.js`).
