...
function fail(msg) {
  console.error('build-designhub.js: ' + msg);
  process.exit(1);
}
...
function replaceOnce(body, anchor, replacement, label) {
  ...
  if (i === -1)
    throw new Error(
      'anchor not found (' +
        label +
        '): upstream feedback-widget.html changed - review and update build-designhub.js'
    );
  if (body.indexOf(anchor, i + 1) !== -1)
    throw new Error(
      'anchor not unique (' +
        label +
        '): the exact text "' +
        anchor +
        '" occurs more than once - upstream duplicated or restructured this code; narrow the anchor and update build-designhub.js'
    );
...
function assertValidJs(content, label) {
  const f = path.join(
    os.tmpdir(),
    'ccfb-designhub-' + label + '-' + process.pid + '-' + Date.now() + '.js'
  );
  try {
...
function transform(src) {
  ...
  if (countOf(/<style>/g) !== 1)
    throw new Error(
      'expected exactly one <style> block in feedback-widget.html, found ' +
        countOf(/<style>/g) +
        ' - upstream added/removed a block; review the new structure before updating the extraction regexes'
    );
  if (countOf(/<script>/g) !== 1)
    throw new Error(
      'expected exactly one <script> block in feedback-widget.html, found ' +
        countOf(/<script>/g) +
        ' - upstream added/removed a block; review the new structure before updating the extraction regexes'
    );
  const grab = (re, label) => {
    ...
    if (!m)
      throw new Error(
        'could not find ' +
          label +
          ' in feedback-widget.html (expected ' +
          re +
          '): upstream structure changed - review and update build-designhub.js'
      );
    return m[1];
  };
  const css = grab(/<style>([\s\S]*?)<\/style>/, '<style> block').trim();
  let markup = grab(/<\/style>([\s\S]*?)<script>/, 'markup between </style> and <script>')
    .trim()
    .replace(/^<!--[\s\S]*?-->\s*/, '');
  ...
  if (!iife)
    throw new Error(
      'source <script> must be a single bare IIFE: (function(){ ... })() - wrapper not found, upstream changed'
    );
  let body = iife[1];
  if (css.length < 100 || markup.length < 100 || body.length < 100) {
    throw new Error(
      'extraction produced suspiciously small output (css ' +
        css.length +
        ', markup ' +
        markup.length +
        ', body ' +
  ...
  if (!/EventSource/.test(sse[0]))
    throw new Error('subscribeSSE block does not contain EventSource - refusing to replace');
  body = replaceOnce(
    body,
    sse[0],
  ...
  if (count !== 9)
    throw new Error(
      'expected exactly 9 location.href sites after clean removal, found ' +
        count +
        ' - upstream changed, re-audit page keying'
    );
...
function wrap(out) {
  return (
    '// GENERATED by designhub/build-designhub.js from upstream feedback-widget.html - NEVER EDIT.\n' +
...
function main() {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (disk !== wrapped)
      fail('designhub/gas/widget.js is stale - run: node designhub/build-designhub.js');
