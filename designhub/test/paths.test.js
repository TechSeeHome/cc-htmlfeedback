...
test('docPath builds repo/featureDir/subpath', () => {
  assert.equal(
    P.docPath('cc-htmlfeedback', 'design/designhub-platform', 'docs/designhub/design.html'),
...
test('docPath and parseDocPath round-trip', () => {
  const built = P.docPath(
    'cc-htmlfeedback',
    'design/designhub-platform',
    'docs/designhub/design.html'
  );
  const parsed = P.parseDocPath(built);
  assert.deepEqual(parsed, {
    repo: 'cc-htmlfeedback',
    featureDir: 'design--designhub-platform',
    segments: ['docs', 'designhub', 'design.html'],
