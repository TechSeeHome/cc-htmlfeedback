...
4. **Agents** - no custom API in v1: **the Sheet is the API.** Agents use Drive/Sheets
   ...
   **write access comes from D13**: publish grants editor on the companion Sheet to the
   `designhub-agents@` group, so a CI bot or a developer who isn't the publisher writes
   as itself after joining the group once.
...
bridge resolves the target Sheet directly from the path instead of scanning every
Sheet for a comment UUID (ambiguous the moment two docs exist).
...
- Agent path: a Claude Code session reads open tickets from a Sheet and posts a reply,
  using only documented contracts - run it as a **non-publisher identity** to prove the
  `designhub-agents@` editor grant (D13) actually authorizes the write.
