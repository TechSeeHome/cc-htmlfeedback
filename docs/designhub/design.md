...
Built as `apps/knowledge-portal` in `home-rnd-productivity-v2`, not this repo - full
spec/design docs live there
(`docs/superpowers/specs/2026-07-12-knowledge-portal-product-review-design.md`,
`2026-07-13-knowledge-portal-slice-b-ingestion-design.md`). What actually shipped,
for anyone reading this repo's own history without that context:
...
- A Manage dialog: upload a `.md`/`.html` doc or register an external link,
  ...
  source" (Drive itself, plus this one dialog for the two sourceless types)
  rather than a bulk one-time importer.
...
- Portal home replacing DesignHub's v1 tree UI, richer facets (tags/team/epic),
  lifecycle/archive states, capability matrices - **not started**, explicitly
  deferred as unnecessary at current scale (single-digit curators, ~160 rows) -
  see that repo's own ingestion-design doc's "Out of scope" section.
