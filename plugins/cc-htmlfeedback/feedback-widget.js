...
<div id="fb-popover">
  ...
  // eslint-disable-next-line no-unused-vars, no-cond-assign, unicorn/no-useless-spread
  ...
  /* ---- draft persistence: sessionStorage, key derived from the normalized path (matches
     ...
     project namespace (window.__CCFB.ns, a stable hash of the queue dir) - without it,
     stopping one /cc-htmlfeedback project and starting another on the same default port+path
     would restore the first project's drafts into the second. ns survives restarts of the
     same project's server; sessionId (fresh per start) is only the fallback for an older
     server that doesn't inject ns. References window.__CCFB directly since CCFB itself
     isn't assigned until below. ---- */
  const DRAFT_KEY = 'ccfb-drafts:' + (function(){
    const ns = (window.__CCFB && (window.__CCFB.ns || window.__CCFB.sessionId)) || '';
  ...
  ta.addEventListener('keydown', e => {
    if(e.key === 'Enter' && !e.shiftKey){ e.preventDefault(); add('comment', e.metaKey || e.ctrlKey); }
    // Held Cmd/Ctrl+Backspace must not spam plain (non-fix-now) strikes once the fast path's own
    // !e.repeat check blocks it - excluding that combination here (rather than a separate branch)
    // keeps the repeat guard and the fixNow forwarding in one condition instead of two.
    else if(e.key === 'Backspace' && ta.value === '' && !((e.metaKey || e.ctrlKey) && e.repeat)){ e.preventDefault(); add('strike', e.metaKey || e.ctrlKey); }
  ...
  // Removes an existing card's DOM node so the next ensureCard() rebuilds it fresh from
  // cardHTML - ensureCard() reuses card DOM and never re-runs cardHTML, so without this the
  // card's chrome (editable vs read-only note, Fix button, status pill) would not follow a
  // draft<->submitted transition.
  function rebuildCard(id){
    const card = list.querySelector('.fb-card[data-fb-id="' + id + '"]');
    if(card) card.remove();
  }
  // Flips draft=false at SEND time (not success) so an in-flight submission is immediately
  // adoptable by reconcile()'s content matcher if SSE beats the response - closing the
  // original race without exposing unsent drafts to that same matcher (see reconcile below).
  function submitDraft(f){
    rebuildCard(f.id);
    f.draft = false; f.status = 'todo'; f.result = ''; f.files = []; f.boardSeen = false;
    render(); schedulePersist();
    return ccfbPost({ type: f.type, quote: f.quote, context: f.context, section: f.section, note: f.note, page: f.page })
      .then(r => { if(!r.ok) throw new Error('http ' + r.status); return r.json(); })
      .then(t => { f.sid = f.sid || t.id; render(); schedulePersist(); return true; })
      .catch(() => {
        rebuildCard(f.id);
        f.draft = true;
        render(); schedulePersist();
        showToast('Submission failed - draft kept', true);
        return false;
      });
  }
  ...
  async function fixAll(){
    ...
    try {
      for(const f of drafts){
        ...
        // rather than silently submitting something the user already tried to cancel.
        if(store[f.id] !== f || f.removed || !f.draft) continue;
        const ok = await submitDraft(f);
        // Kept-as-drafts is a live count, not drafts.length - sent - skipped: a snapshot arithmetic
        // count would still count an item discarded/cleared after this failure as "kept as a draft".
  ...
  function cardHTML(f){
    ...
    const st = statusOf(f);
    const isDraft = CCFB && f.draft;
    const pill = (st && st !== 'draft') ? '<span class="fb-status ' + stClass(st) + '">' + esc(st).replace(/-/g, ' ') + '</span>' : '';
    const pageLbl = (CCFB && f.page && !isDraft) ? '<div class="fb-page" title="' + esc(f.page) + '">' + esc(shortPage(f.page)) + '</div>' : '';
    const quoteSnippet = clip(f.quote, 40);
    const fixBtn = isDraft
      ...
      : '';
    // Draft notes are editable (disconnected-style contenteditable), always rendered even when
    // empty - an empty-note draft (e.g. a strike) needs somewhere to type. A submitted note in
    // connected mode is the read-only instruction already sent (refine via a new comment).
    const note = (CCFB && !isDraft)
  ...
  list.addEventListener('click', e => {
    const fixBtn = e.target.closest('.fb-fixbtn');
    if(fixBtn){ e.stopPropagation(); const f = store[+fixBtn.closest('.fb-card').dataset.fbId]; if(f) submitDraft(f); return; }
  ...
  function reanchor(f){
    ...
    while((n = w.nextNode())){
      ...
      try {
        ...
        f.anchorLost = false;
        return true;
      } catch{ /* overlaps markup - try the next match */ }
    }
    f.anchorLost = true;
    return false;
  }
  ...
  function reconcile(tickets){
    ...
    tickets.forEach(t => {
      ...
      if(!f){
        ...
        // creating a duplicate card + duplicate on-page mark if SSE arrives before the POST
        // resolves. !x.draft is required: without it, an UNSENT draft that merely shares text
        // with a ticket submitted elsewhere (another tab, a duplicate note) would be wrongly
        // adopted and marked as sent - found in review, this guard is not optional. type is
        // compared too: a comment and a strike on the same quote with a matching (often empty)
        // note are otherwise indistinguishable, and cross-matching them swaps their outcomes.
        f = Object.values(store).find(x => !x.draft && !x.sid && x.quote === (t.quote || '') && x.note === (t.note || '') && x.page === (t.page || '') && x.type === (t.type || 'comment'));
        if(f) f.sid = f.sid || t.id;
      }
      if(!f){
        const id = ++uid;
  ...
  function subscribeSSE(){
    try {
      ...
      es.onerror = () => {
        if(es.readyState === 2){ setConn('offline'); try { es.close(); } catch{}; setTimeout(subscribeSSE, 5000); }  // CLOSED: browser won't retry - reconnect ourselves
  ...
  function maybeBanner(){
    let dismissed = false; try { dismissed = !!localStorage.getItem('ccfb-banner-dismissed'); } catch{}
